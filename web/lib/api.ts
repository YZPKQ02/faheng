...
export type Authority = { id: string; title: string; article: string; content: string; level: string; effective_on: string; expired_on?: string | null; source_url: string };
...
export type Conclusion = { id: string; viewpoint: string; counterargument: string; confidence: number; uncertainties: string[]; authority_ids: string[]; reasoning_trace: ReasoningTrace[]; quality_metrics: Record<string, unknown>; is_current: boolean; invalidated_reason?: string | null; publication_status: "draft" | "pending_review" | "published" | "blocked" | "rejected" | "changes_requested" | "stale"; published_at?: string | null; published_by?: string | null };
...
export type SimulationCompletionReason = "natural_end" | "user_ended" | "max_rounds" | "superseded";
export type SimulationLine = { role: string; content: string; agent_id?: "system" | "arbitrator" | "employer_advocate" | "worker"; mode?: "model" | "rule" | "hybrid"; mode_reason?: string | null; stage?: string; round_number?: number; last_execution?: string[]; fallback_agents?: string[]; completion_reason?: SimulationCompletionReason | null; kind?: "question" | "closing"; stream_id?: string };
export type Simulation = { id: string; scenario: string; user_role: string; transcript: SimulationLine[]; feedback: string[]; suggested_answers: string[]; assistance_mode: "coach"; counsel_agent_id: string; counsel_memory_version: number; counsel_memory_snapshot: Record<string, unknown>; status: "active" | "completed"; completion_reason: SimulationCompletionReason | null; completed_at: string | null; created_at: string; updated_at: string };
...
async function buildHeaders(headers?: HeadersInit) {
  ...
  return { ...merged, ...(await authHeaders()) };
}
...
export const api = {
  ...
  getAuthorities: (id: string) => request<Authority[]>(`/cases/${id}/authorities`),
  ...
  analyze: (id: string) => request<{ conclusions: Conclusion[]; authorities: Authority[]; evidence_gaps: string[]; next_steps: string[]; disclaimer: string; requires_human_review: boolean; blocked_reasons: string[] }>(`/cases/${id}/analysis`, { method: "POST", body: JSON.stringify({}) }),
